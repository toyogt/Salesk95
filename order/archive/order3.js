...
class K95OrderApp {
    ...
    // Enhanced detectLocation with 5 retries
    async detectLocation() {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                console.log(`📍 Location detection attempt ${attempt}/${maxRetries}`);
                ...
                document.getElementById('locationMapUrl').value = mapUrl;
                
                // Try to get address (optional, may fail due to CORS)
                try {
                    const address = await this.reverseGeocodeWithRetry(latitude, longitude);
                    document.getElementById('newOutletLocation').value = address;
                    this.showMessage('📍 Location detected with address!', 'success');
                } catch (addressError) {
                    console.warn('⚠️ Reverse geocoding failed, using coordinates only');
                    document.getElementById('newOutletLocation').value = 
                        `Latitude: ${latitude.toFixed(6)}, Longitude: ${longitude.toFixed(6)}`;
                    this.showMessage('📍 Coordinates captured. Address lookup unavailable.', 'success');
                }
                ...
                if (attempt < maxRetries) {
                    ...
                    this.showMessage(`Retrying location detection (${attempt}/${maxRetries})...`, 'success');
        ...
        // All retries failed
        this.showMessage(`❌ Location detection failed after ${maxRetries} attempts. Please enter manually.`, 'error');
    }
    ...
    // Helper method for reverse geocoding with retry
    async reverseGeocodeWithRetry(lat, lon, retries = 3) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                const response = await fetch(url, { 
                    headers: { 'User-Agent': 'K95Foods/1.0' },
                ...
                console.warn(`⚠️ Reverse geocode attempt ${attempt} failed:`, error.message);
    ...
    async submitOrder(mode) {
        ...
        try {
            ...
            // IMPORTANT FIX: Deduct inventory ONLY ONCE with a transaction ID
            if (mode === 'submit' && orderData.fulfillment_type === 'Distributor') {
                console.log('🔍 Starting inventory deduction for order:', orderNumber);
                
                // Use a Set to track processed products for this submission
                const processedInThisSubmission = new Set();
                
                for (const item of Object.values(this.orderItems)) {
                    // Create a unique key for this product in this order
                    const deductionKey = `${order.id}_${item.product_id}`;
                    
                    if (processedInThisSubmission.has(deductionKey)) {
                        console.warn('⚠️ Duplicate deduction attempt blocked for', item.product_name);
                        continue;
                    }
                    processedInThisSubmission.add(deductionKey);
                    
                    console.log(`Deducting ${item.qty} from ${item.product_name}`);
                    ...
                    if (error) {
                        ...
                        console.log(`✅ Deducted ${item.qty} from ${item.product_name}`);
                    }
                }
                
                // Refresh local inventory
                await this.loadInventory(this.currentDistributor.distributor_id);
                console.log('✅ Inventory refreshed after deduction');
            }
            ...
            if (submitBtn) submitBtn.disabled = false;
            this.submitting = false;
            console.log('🔓 Order submission completed, buttons re-enabled');
        }
    }
    
    ///////////////////////test///////////////////////
    // Keep a Set of processed products for this submission
    const processedProducts = new Set();
    
    if (mode === 'submit' && orderData.fulfillment_type === 'Distributor') {
        for (const item of Object.values(this.orderItems)) {
            const key = `${item.product_id}_${item.qty}_${Date.now()}`;
            ...
            console.log('Calling decrement_inventory for', item.product_name, 'qty:', item.qty);
            const { error } = await supabaseClient.rpc('decrement_inventory', {
                p_distributor_id: this.currentDistributor.distributor_id,
                p_product_id: item.product_id,
                p_qty: item.qty
